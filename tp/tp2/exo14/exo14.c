...
char* remplacer_nombres_par_i(const char* input) {
    ...
    int j = 0; // Indice pour la chaîne de sortie (output)
    for (int i = 0; i < len; ++i) {
...
int main(int argc, char *argv[]) {
    char *input=argv[1]; // Déclaration de la chaîne d'entrée
    strcat(input, "$"); // Ajout du marqueur de fin de chaîne
    char* input2 = remplacer_nombres_par_i(input);
    printf("chaine %s\t",input2);
    parse(input2); // Appel de la fonction d'analyse
